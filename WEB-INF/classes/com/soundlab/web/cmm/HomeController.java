...
public class HomeController {
	...
	@RequestMapping(value = "/main", method = RequestMethod.GET)//get방식을 생략할수있고 생략하면 value도 생략할수있어
	public String main(Model model, HttpServletRequest request) {//request가 모델안에 있게된다.(pom.xml에서 정의된(전역으로) 모델이 돌아다닌다)
		logger.info("Hello SoundLAB main! :::: 한글아 깨지니??");
		model.addAttribute("context", Util.ctx.apply(request));
		Util.log.accept("context ::: " +Util.ctx.apply(request));
		return "main";
	}
