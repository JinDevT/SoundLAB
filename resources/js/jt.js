...
jt ={
		...
		music_player : x=>{
			...
			$.getJSON($.ctx()+'/service/player/music/'+x+'/'+memberId,d=>{
				let openWin;
				openWin = window.open(sh.ctx()+'/main/#',"soundlabPlayer","left="+(screen.availWidth-730)/2+",top="+(screen.availHeight-495)/2+","+"width=730,height=495, menubar=no");
		...
		album_player : x=>{
			...
			$.getJSON($.ctx()+'/service/player/album/'+x+'/'+memberId,d=>{
				let openWin = window.open(sh.ctx()+'/main/#',"soundlabPlayer","left="+(screen.availWidth-730)/2+",top="+(screen.availHeight-495)/2+","+"width=730,height=495, menubar=no");
				let player = $(openWin.document.getElementById('wrapper')).length;
				if(player == 0){
					openWin.onload =(()=>{
		            	setTimeout(x=>{
